...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/health') return json({ ok: true, service: 'uniq-smart-rent', d1: Boolean(env.DB), d1Ready: Boolean(env.DB), schemaVersion: env.DB ? 12 : null, verifiedCatalog: vehicles.length, ownerFleetManagement: true, bookingCalendar: true, rentalLifecycle: true, paymentCheckout: true, paymentProviders: 7, employeesBranches: true, vehicleTransfers: true, ownerFinance: true, refunds: true, deposits: true, serviceManagement: true, serviceInspections: true, serviceExpenses: true, marketingCampaigns: true, promotionsMarketing: true, marketingSegments: true, ownerAnalytics: true, analyticsFunnel: true, vehicleProfitability: true, demoRequestDataset: true, dynamicBranches: true, clientBookingExtension: true, balancePayments: true, overpaymentProtection: true, fullOperationalFleet: true }, 200, corsHeaders);
