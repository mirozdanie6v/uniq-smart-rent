...
export default{async fetch(request:Request,env:AutoSaleEnv):Promise<Response>{
  ...
  if(url.pathname==='/api/health')return json({ok:true,service:'auto-sale-usa',productMode:env.PRODUCT_MODE||'auto-sale-usa',d1:Boolean(env.DB),schemaVersion:5,demoMode:env.AUTO_SALE_DEMO_MODE==='1'});
