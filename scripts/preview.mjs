...
createServer(async (req,res)=>{
  try {
    ...
    if(pathname.startsWith('/api/')){res.writeHead(503,{'content-type':'application/json'});res.end(JSON.stringify({error:'preview_api_unavailable'}));return;}
    const candidate=path.join(root,pathname==='/'?'index.html':pathname.replace(/^\/+/,''));
    let target=candidate; try{if(!(await stat(target)).isFile())throw new Error('not-file');}catch{target=path.join(root,'index.html');}
