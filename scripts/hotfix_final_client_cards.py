...
close_new='<button className="crm-close" aria-label="Закрыть карточку клиента" onClick={closeCustomer}>×</button>{historyFocusId === selected.id ? <div className="crm-history-focus-head" data-rental-history-focus><span className="eyebrow">ИСТОРИЯ АРЕНД</span><h2>{selected.name}</h2><button type="button" className="secondary" data-back-customer-card onClick={() => setHistoryFocusId(null)}>← Карточка клиента</button></div> : <button type="button" className="crm-history-launch" data-open-rental-history onClick={() => openRentalHistory(selected.id)}>История аренд →</button>}'
...
text=p.read_text(encoding='utf-8')
text += "\n.crm-history-launch{position:absolute;left:20px;top:16px;z-index:3;border:0;background:transparent;color:#adffc8;padding:7px 0;font:inherit;font-size:11px;font-weight:850;cursor:pointer;text-decoration:underline;text-underline-offset:3px}.crm-profile-head{margin-top:38px}.crm-history-focus-head{padding:2px 42px 12px 0;min-height:78px}.crm-history-focus-head h2{margin:6px 0 12px;font-size:23px}.crm-history-focus-head .secondary{min-height:38px;padding:0 12px;font-size:10px}.crm-detail.history-focused .crm-profile-head,.crm-detail.history-focused .crm-profile-stats,.crm-detail.history-focused .crm-contact-grid,.crm-detail.history-focused .crm-edit-field,.crm-detail.history-focused .crm-timeline-head{display:none}.crm-detail.history-focused .crm-timeline{margin-top:4px}.crm-detail.history-focused .crm-timeline article:first-child{padding-top:0}@media(max-width:700px){.crm-history-launch{position:sticky;display:inline-flex;top:0;left:auto;margin:-4px 48px 10px 0;padding:10px 12px;border:1px solid rgba(88,214,145,.2);border-radius:12px;background:#11211e;text-decoration:none}.crm-profile-head{margin-top:0}}\n"
p.write_text(text,encoding='utf-8')
