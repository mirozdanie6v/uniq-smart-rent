...
async function saveImage(url,base){const r=await fetchTimed(url,{headers:{referer:`${SITE}/ru`}});const ct=r.headers.get('content-type')||'';if(!ct.startsWith('image/'))throw new Error(`Not image ${ct}`);const target=base+ext(url,ct);await writeFile(target,Buffer.from(await r.arrayBuffer()));return './'+path.relative(process.cwd(),target).replaceAll('\\','/')}
