...
test('AUTO SALE has isolated D1 entity tables and schema 6 demo cleanup',()=>{
...
test('Worker exposes schema 6 D1 state API and virtual demo layer',()=>{
  assert.match(worker,/\/api\/auto-sale\/state/);assert.match(worker,/schemaVersion:6/);assert.match(worker,/withDemoBusiness/);assert.match(worker,/syncState/);
});
