...
const isDemoId=value=>/(^|-)DEMO(-|$)/i.test(String(value??''));
function mergeDemoRows(serverRows=[],localRows=[]){const map=new Map((Array.isArray(serverRows)?serverRows:[]).map(x=>[String(x?.id||''),x]));for(const item of Array.isArray(localRows)?localRows:[])if(item&&isDemoId(item.id))map.set(String(item.id),item);return [...map.values()]}
function mergeDemoNotes(serverNotes={},localNotes={}){const merged={...(serverNotes&&typeof serverNotes==='object'?serverNotes:{})};if(localNotes&&typeof localNotes==='object')for(const [leadId,value] of Object.entries(localNotes))if(isDemoId(leadId))merged[leadId]=value;return merged}
function applyServerState(state){if(!state||!state.initialized)return;const keepDemo=Boolean(state.demoBusiness);const localLeads=keepDemo?readCache(DATA_KEYS.leads,[]):[],localQuotes=keepDemo?readCache(DATA_KEYS.quotes,[]):[],localOrders=keepDemo?readCache(DATA_KEYS.orders,[]):[],localNotes=keepDemo?readCache(DATA_KEYS.notes,{}):{};writeCache(DATA_KEYS.leads,keepDemo?mergeDemoRows(state.leads||[],localLeads):state.leads||[]);writeCache(DATA_KEYS.quotes,keepDemo?mergeDemoRows(state.quotes||[],localQuotes):state.quotes||[]);writeCache(DATA_KEYS.orders,keepDemo?mergeDemoRows(state.orders||[],localOrders):state.orders||[]);writeCache(DATA_KEYS.notes,keepDemo?mergeDemoNotes(state.notes||{},localNotes):state.notes||{})}
...
await import('./auto-sale-lead-save-fix.mjs');
