...
function fallbackSaveLead(form,button){
  ...
  if(i<0){resetButton(button);feedback(form,'Карточка устарела и больше не найдена в общей базе. Перезагрузите страницу и откройте её заново.','error');return}
...
window.addEventListener('auto-sale-server-synced',()=>{
  ...
  feedback(form,'Сохранено в общей базе D1.','success');
  if(form.dataset.fallbackSaved==='1'){form.dataset.fallbackSaved='0';setTimeout(()=>location.reload(),550)}
});
window.addEventListener('auto-sale-server-rejected',event=>{const form=document.querySelector('#leadEditForm,#quoteForm');if(!form)return;resetButton(buttonFor(form));const detail=event.detail||{},extra=Array.isArray(detail.details)&&detail.details.length?`: ${detail.details.join(', ')}`:'';feedback(form,`Сервер не принял изменения (${detail.error||'ошибка проверки'})${extra}. Исправьте данные и сохраните ещё раз.`,'error')});
window.addEventListener('auto-sale-server-conflict',()=>{const form=document.querySelector('#leadEditForm,#quoteForm');if(form){resetButton(buttonFor(form));feedback(form,'Данные были изменены в другой сессии. Загружена актуальная версия; проверьте поля перед повторным сохранением.','warn')}});
