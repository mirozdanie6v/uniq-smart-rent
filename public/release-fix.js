(() => {
  ...
  function analyticsData(){
    ...
    for(const item of rows){const key=item.sourceChannel||item.source||'telegram_mini_app';const row=sources.get(key)||{count:0,revenue:0};row.count++;if(pay[item.id]?.status==='paid')row.revenue+=Number(pay[item.id]?.amount)||0;sources.set(key,row)}
    const byVehicle=new Map();
    for(const item of rows){const row=byVehicle.get(item.vehicleId)||{count:0,revenue:0};row.count++;if(pay[item.id]?.status==='paid')row.revenue+=Number(pay[item.id]?.amount)||0;byVehicle.set(item.vehicleId,row)}
  ...
  document.addEventListener('click',event=>{
    ...
    if(fleetCount){event.preventDefault();document.querySelector('.bottom-nav [data-go="catalog"]')?.click();return}
    const pay=event.target.closest?.('[data-pay-request]');
    if(pay)setTimeout(()=>openPayment(pay.dataset.payRequest),0);
    const baseNav=event.target.closest?.('[data-go]');
    if(baseNav)analyticsOpen=false;
    if(event.target.closest?.('[data-role]'))analyticsOpen=false;
  },true);
  document.addEventListener('keydown',event=>{
    if((event.key==='Enter'||event.key===' ')&&event.target?.matches?.('[data-release-catalog]')){event.preventDefault();event.target.click()}
  });
